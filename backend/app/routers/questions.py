...
@router.get("/courses")
def list_courses(s: Session = Depends(get_db)):
...
def get_question(question_id: int, s: Session = Depends(get_db)):
...
def questions_for_course(slug: str, s: Session = Depends(get_db)):
...
def get_question_pdf(question_id: int, s: Session = Depends(get_db)):
    q = s.get(Question, question_id)
